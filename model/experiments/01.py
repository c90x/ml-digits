"""
01: Simple Logistic Regression Model
"""
...
name = "01_simple_logistic_regression"
    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for inputs, targets in loader_train:
            # DigitDataset labels are plain Python ints → cast to LongTensor
            # (CrossEntropyLoss requires targets as torch.long)
...
def main():
    ...
    train_result = train(
        model, dataset_train=dataset_train, dataset_validate=dataset_validate, epochs=50
    )
    ...
    torch.save(
        model.state_dict(),
        experiment_path / "model_epoch50.pt",
    )

    with open(experiment_path / "results.json", "w") as file:
        json.dump({"train": train_result, "eval": eval_result}, file, indent=4)
